...
public class BannerAdRoller {
    ...
    private static final int MAX_ROLLER = 20;
        ...
        this.timer.schedule(new TimerTask() {
            public void run() {
                ...
                if (BannerAdRoller.this.isBannerShownEnoughTime(adapter)) {
                    Logger.debug(TAG, "banner has shown enough time, signal the banner refresh thread...");
                ...
                if (!BannerAdRoller.this.mStopAdRoller) {
                    ...
                    while (!BannerAdRoller.this.mStopAdRoller) {
                        ...
                        if (rollerIteration2 > MAX_ROLLER) {
                            Logger.debug(TAG, "reach max roller, stop ad roller");
...
//              }
                            ...
                            Logger.debug(TAG, "BannerAdRoller selectAd");
                        ...
                        if (BannerAdRoller.this.mStopAdRoller) {
                            if (adapter != null) {
                                Logger.debug(TAG, "Banner was loaded but ad roller was stopped. So we save it for later use.");
                                BannerAdRoller.this.setPreviousAdapter(BannerAdRoller.this.mAdapter);
...
//              }
                            ...
                            BannerAdRoller.this.mLock.lock();
                            try {
                                Logger.debug(TAG, "Sleeping, wait to signal awake");
                        ...
                        try {
                            if (BannerAdRoller.this.mStopAdRoller) {
                                Logger.debug(BannerAdRoller.TAG, "AdRoller stopped");
                            } else if (adapter == null) {
                    ...
                    Logger.debug(TAG, "Ad Roller is stopped");
                }
            }
        });
    }
