...
public abstract class WaterfallAdSelector<T extends BaseConfig> implements AdSelectorCallback, AdSelector<T> {
  ...
  private final List<BaseAdapter> mPreviouslyLoadedAdapterList = new ArrayList<>();
    ...
    try {
      ...
      while (index < totalSize) {
        ...
        if (adapter.isReadyForSelect()) {
          ...
          Logger.debug(TAG, adapter.getName() + " getReadyAdapter");
          return adapter;
        }
        index++;
      }
  ...
  public BaseAdapter selectAd(Activity activity, T config, List<BaseAdapter> gridAndRegisteredProviders) {
    if (gridAndRegisteredProviders.size() == 0) {
      Logger.debug(TAG, "No adapter for " + mAdType);
      return null;
    }
  ...
  public void adLoadSuccess(BaseAdapter adapter) {
    ...
    Logger.debug(TAG, "Great, " + mAdType.name() + adapter.getName() + " notify loaded success");
    this.mLock.lock();
    try {
  ...
  private BaseAdapter fetchAd(final Activity activity, T config, List<BaseAdapter> gridAndRegisteredProviders, boolean force) {
    Logger.debug(TAG, "fetch new " + mAdType.name() + " started");
    ...
    if (mAdType != IvyAdType.BANNER && mAdType != IvyAdType.NATIVE_AD) {
      while (index < gridAndRegisteredProviders.size()) {
        ...
        if (adapter.isReadyForSelect()) {
          mCurrentLoadedAdapter = adapter;
          Logger.debug(TAG, adapter.getName() + " is loaded, fulfill this request with this adapter");
    ...
    try {
      ...
      while (index < gridAndRegisteredProviders.size()) {
        ...
        if (adapter.isFetching()) {
          index++;
          Logger.debug(TAG, "Adapter " + adapter.getName() + " is still in fetching, try next adapter");
          continue;
        }
        ...
        if (this.mCycleBanners && this.mPreviouslyLoadedAdapterList.contains(adapter)) {
          ...
          Logger.debug(TAG, "[WATERFALL] waterfall skipped " + adapter.getName() + ": cycle banners");
        } else if (!force && adapter.getSelectedWeight() == 0) {
          adapter.skipFetch(AdapterSkipReason.SKIP_ZERO_WEIGHT);
          Logger.debug(TAG, "[WATERFALL] waterfall skipped " + adapter.getName() + ": zero weight");
          Logger.debug(TAG, "Adapter " + adapter.getName() + " disable by zero weight");
        } else if (adapter.isForceSkippedByCountry()) {
          adapter.skipFetch(AdapterSkipReason.SKIP_BY_COUNTRY);
          Logger.debug(TAG, "[WATERFALL] waterfall skipped " + adapter.getName() + ": country specified");
        } else if (force || !adapter.isForceSkipped()) {
          debugMsg = adapter.getName() + ":" + this.mAdType + " trying to load";
          Logger.debug(TAG, "[WATERFALL] waterfall skipped " + adapter.getName() + ": force skipped");
          Logger.debug(TAG, debugMsg);
          adapter.resetOperationCount();

          Logger.debug(TAG, "Putting " + adapter.getName() + " in loading queue");
          this.mCurrentLoadingAdapter = adapter;

          getUiHandler().post(() -> adapter.fetch(activity, WaterfallAdSelector.this));

          Logger.debug(TAG, "Adapter " + adapter.getName() + " waiting " + waitingLoadingTimeout + " seconds for the loading result...");
          if (!this.mCond.await(waitingLoadingTimeout, TimeUnit.SECONDS)) {
            ...
            Logger.debug(TAG, "We are loading " + adapter.getName());
          ...
          Logger.debug(TAG, "Waterfall skipped: " + reason);
        }
        index++;
      }
      this.mLock.unlock();
    ...
    if (adProvider == null && gridAndRegisteredProviders.size() != 0) {
      ...
      Logger.debug(TAG, debugMsg + gridAndRegisteredProviders);
    } else if (gridAndRegisteredProviders.size() == 0) {
      debugMsg = "For " + this.mAdType + ", there are no providers registered";
      Logger.warning(TAG, debugMsg);
    } else {
      Logger.debug(TAG, "Fulfill " + mAdType.name() + " this ad with: " + adProvider.getName());
    }
