...
public abstract class BaseAdapter<T extends BaseAdapter.GridParams> implements Adapter<T> {
  ...
  // 此Adapter后台配置的估算ecpm
  private double estimatedEcpm = 0;
  ...
  public void setCountrySpecified(JSONObject payload) {
    ...
    if (this.forceSkippedByCountry) {
      Logger.debug(TAG, "Adapter " + this + " skipped by country specified settings");
    }
  }
  ...
  public boolean isFetching() {
    if (mLoadStartTime > 0) {
      ...
      if (notResponseSecs >= 60) {
        Logger.debug(TAG, mAdType.name() + " Adapter " + getName() + " not response for " + notResponseSecs + "s, reset to loaded failed status");
  ...
  public boolean isSleeping() {
    ...
    if (sleeping) {
      Logger.debug(TAG, "Adapter is sleeping ,will awake in " + (mSleepEndTimes - System.currentTimeMillis()) / 1000 + "seconds");
    }
    return sleeping;
  }
  ...
  public void fetch(@NonNull Activity activity, AdSelectorCallback callback) {
    Logger.debug(TAG, this.getAdType().name() + ", " + this.getName() + " fetch, begin");
    // 广告正在被加载, 忽略本次fetch的请求, 需要等待适配loaded failed, loaded success的状态
    if (mAdapterStatus == STATUS_FETCHING) {
      Logger.debug(TAG, "Adapter: " + this.getClass().getName() + " is fetching, waiting the adapter load result");
      return;
    }
    ...
    if (mAdapterStatus == STATUS_LOADED_SUCCESS) {
      Logger.debug(TAG, "Adapter" + this.getClass().getName() + " already in loaded success status, just do the callback");
      onAdLoadSuccess();
      return;
    }
  ...
  public void skipFetch(String reason) {
    Logger.debug(TAG, "Skipping ad provider: '%s' for reason: '%s' / '%s'", getName(), reason, reason);
  }
  ...
  public void softPause() {
    final BaseAdapter<T> mBaseAdapter = this;
    this.mUiHandler.post(() -> {
      Logger.warning(BaseAdapter.TAG, "%s softPause called, mAdopenCloseCallback is null = %s", BaseAdapter.this.getName(), BaseAdapter.this.mAdOpenCloseCallback == null);
  ...
  public void softResume() {
    final BaseAdapter<T> mBaseAdapter = this;
    this.mUiHandler.post(() -> {
      Logger.warning(BaseAdapter.TAG, "%s softResume called, mAdopenCloseCallback is null = %s", BaseAdapter.this.getName(), BaseAdapter.this.mAdOpenCloseCallback == null);
      if (BaseAdapter.this.mAdOpenCloseCallback != null) {
  ...
  public void markSleepSecs(int sleepSeconds) {
    long realSleepEndTimes = System.currentTimeMillis() + sleepSeconds * 1000L;
  ...
  public void onAdShowFail() {
    Logger.debug(TAG, "Adapter " + mGridName + " show failed");
  ...
  public void setEstimatedEcpm(double currentEcpm) {
