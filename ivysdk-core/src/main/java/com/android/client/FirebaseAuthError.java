...
public enum FirebaseAuthError {
  ...
  public static FirebaseAuthError fromException(Exception ex) {
    try {

      if (ex instanceof FirebaseAuthException) {
        return FirebaseAuthError.valueOf(((FirebaseAuthException) ex).getErrorCode());
      }  else if (ex instanceof FirebaseNetworkException) {
        return FirebaseAuthError.ERROR_NETWORK;
      }  else if (ex instanceof FirebaseTooManyRequestsException) {
        return FirebaseAuthError.ERROR_TOO_MANY_REQUESTS;
      } else if (ex instanceof FirebaseApiNotAvailableException) {
        return FirebaseAuthError.ERROR_API_NOT_AVAILABLE;
      } else {
        Logger.error("FirebaseAuthError", "exception: " + ex.getClass().getName(), ex);
