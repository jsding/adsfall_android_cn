...
@Keep
public class PurchaseManagerImpl implements PurchaseManager, EventListener, ApiCallback {
  ...
  public void buy(@NonNull String productId, @Nullable String developerPayload) {
    try {
      ...
      SKUDetail skuDetail = getSKUDetail(productId);

      int amount = (int)(skuDetail.getUsd() * 100);

      PayInfo payInfo = new PayInfo(purchasingOrderId, null, amount);
      String productDest = skuDetail.getDescription();
      payInfo.setProductDesc(productDest);
      String productName = skuDetail.getTitle();
      payInfo.setProductName(productName);
      payInfo.setType(PayInfo.TYPE_NOARMAL_PAY);
      payInfo.setCurrency("USD");
      payInfo.setAttach(uid);
      payInfo.setCallbackUrl(this.preferences.callbackUrl);

      GameCenterSDK.getInstance().doPay(activity, payInfo, this);
    } catch (Throwable e) {
  ...
  public void onSuccess(String resultMsg) {
    Logger.debug(TAG, "purchase success, message" + resultMsg);

    // 支付成功
    processPurchase(this.purchasingOrderId, purchasingItemId);
  }

  @Override
  public void onFailure(String resultMsg, int resultCode) {
    // 支付失败
    Logger.error(TAG, "purchase failure " + resultCode + ", code " + resultCode);
    purchaseStateChange(PurchaseState.ERROR, this.purchasingOrderId, purchasingItemId);
  }
}
